...
/*   Updated: 2020/11/27 20:45:56 by lle-briq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	size_tot(t_print param, char *nb_utox)
{
	int		max1;
	int		max2;

	max1 = ft_max(param.field, param.precision);
	max2 = ft_max(max1, ft_strlen(nb_utox));
	return (max2 + 1);
}
...
int		print_6hex(t_print param, va_list args)
{
	...
	if (!to_print)
		return (0);
	fill_str(&to_print, nb_utox, param, size);
	free(nb_utox);
	ft_putstr(to_print);
	free(to_print);
	return (size - 1);
